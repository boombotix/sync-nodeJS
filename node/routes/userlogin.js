	...
	var newReg = 0;
			...
			newReg = 1;
                    ...
                    checkAppVersion(userId, appVersion, function(updateAppPopUp)
                    {
                        ...
                        {
                            var personalData = [];
                            personalData.push({"user_id": userId, "user_name": results[0].user_name, "user_access_token": results[0].user_access_token, "user_image": results[0].user_image, "session_exist": sessionResult, "is_premium": results[0].is_premium, "broadcast_secs": secs, "expires_in": tResult, "new_reg": newReg, "popup": updateAppPopUp, "popup_status": popupStatus});
            ...
            else
            {
                ...
                if (results[0].user_active == 0) {
                    ...
                    checkAppVersion(userId, appVersion, function(updateAppPopUp)
                    {
                        ...
                        var personalData = [];
                        personalData.push({"user_id": userId, "user_name": results[0].user_name, "user_access_token": results[0].user_access_token, "user_image": userImageBaseUrl + results[0].user_image, "session_exist":0, "is_premium": results[0].is_premium, "broadcast_secs": 600, "expires_in":'23:59:59', "new_reg": 1, "popup": 0, "popup_status": popupStatus});
                        response1 = {"personal_data": personalData};
                        var resultData = [];
                        resultData.push(response1);
                        var response = {"data": resultData}

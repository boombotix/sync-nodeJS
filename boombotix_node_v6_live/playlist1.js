                ...
                connection.query(sql, function(err, responseSong) {
                    ...
                    if (songsCount == songsStatusCount)
                    {
                                ...
                                connection.query(sql, [userId], function(err, responseSessionId) {
                                    ...
                                    for (var a = 0; a < goodSongsLength; a++)
                                    {
                                        (function(a) {
                                            ...
                                            var n = indexes[a];
                                            savingPlaylistInDb(flag, userId, songName[n], songArtist[n], songImageUrl[n], songLink[n], songItunesLink[n], sessionId, songStatus, function(songId)
                                            {
                                    ...
                                    if (songLink.length == 1)
                        {
                            ...
                            response1 = {"log":"Song not added to playlist.", "song_id": sonLink1};
                            res.send(JSON.stringify(response1));
                        }
                        else if (songLink.length > 1)
                        {
                            
                            response1 = {"log":"Multiple songs not added to playlist.", "song_id": sonLink1};
                            res.send(JSON.stringify(response1));
                        } 
                                }
                    }
                    else
                    {
                        ...
                        curlSongs = songLink.diff(goodSongs);

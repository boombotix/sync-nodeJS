...
//			}
                    ...
                    savingPlaylistInDb(flag, userId, songName[i], songArtist[i], songImageUrl[i], songLink[i], songItunesLink[i], sessionId, songStatus, function(songId)
                    {
                        ...
                        if(songsCount==1)
                            {
                        response1 = {"log": "Song added to playlist successfully", "session_id": sessionId, "song_id": songId};
                        res.send(JSON.stringify(response1));
                            }
                            else
                                {
                                  response1 = {"log":"Multiple songs added to playlist successfully", "session_id": sessionId, "song_id": songId};
                                    res.send(JSON.stringify(response1));  
                                }
                            }
                    });
...
//var command = 'curl -sL -w "%{http_code} %{time_total}\\n" "http://query7.com" -o /dev/null'
    ...
    child = exec(command, function(error, stdout, stderr) {
        ...
        {
            var output = stdout.match("Location:(.*)\r\nServer:");
            ...
            else
            {
               ...
               return callback(output[1].trimLeft());

...
class adminUser {
    ...
    public function getAllUsers($userName) {
        ...
        foreach ($response['data'] as $key => $row) {
                $score[$key] = ($row['is_playing']);
            }

            array_multisort($score, SORT_DESC, $response['data']); 
...
public function playBotPlaylist($data = array()) {
        ...
        if ($retResult == 1) {
	     ...
	     $sql = "UPDATE tb_users set is_premium=? Where user_id=? LIMIT 1";
            $bindParams = array(1, $djId);
            $rowUpdate = $this->_dalObj->sqlQuery($sql, $bindParams);
